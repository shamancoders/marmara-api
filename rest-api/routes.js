...
function getSessionDbId(member, req) {
	return new Promise((resolve, reject) => {
		...
		if (sessionId == '')
			return reject({  name: 'SESSION_NOT_FOUND', message: 'The session has been terminated. Please login.' })
		db.sessions.findOne({ _id: sessionId })
			.then(doc => {
				if (dbnull(doc, reject)) {
					if (doc.passive)
						return reject({  name: 'SESSION_NOT_FOUND', message: 'The session has been set passive. Please login.' })

					if (doc.memberId != member._id)
						return reject({  name: 'INCORRECT_DATA', message: 'Incorrect sessionId ' })

					if (doc.dbId == '')
						return reject({  name: 'SESSION_ERROR', message: 'Session does not have databaseId' })
...
function sendError(err, res) {
	...
	let error = { name: 'Error', message: 'Error' }
